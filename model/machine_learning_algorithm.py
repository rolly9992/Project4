...
#from SPY_Returns_by_Quarter import spy_returns_by_quarter as SPYreturn
...
def train_classifier_model(df,sequence):
    ...
    #tried precision, avg precision
    clfgrid = GridSearchCV(clf, parameters,scoring='roc_auc',cv=5, n_jobs=-1)
...
#############################################################
#temp comment out to work on regressor 
#get model to use for predictions in the following quarter training on particular sequence

def create_ticker_subset_from_ML(sequence,num_tickers_invest=10,df=df):
...
StockFilter = create_ticker_subset_from_ML(sequence=0)


#TODO make a cumulative to account for multiple quarters as the 2 paths could diverge 
def compare_Spy_vs_Spy(StockFilter=StockFilter,SPY=SPY,sequence=1):
    #SPY_= SPY[SPY['return']==sequence]
    #SPYstart = starting_money
    SPYend = SPY['Spy Return'].iloc[sequence]* starting_money
    Stockstart = StockFilter['starting_money'].sum()
    Stockend = StockFilter['ending_money'].sum()
    print('SPY results')
    print({'SPY Start':starting_money,
           'SPY End':starting_money+SPYend,
...
compare_Spy_vs_Spy(StockFilter=StockFilter,SPY=SPY,sequence=1)
